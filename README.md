...
Context API simplifies state management, we can pass state any direction we want without prop drilling.

- [X] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Reducer, reducers are functions that take current state and return a new state
Actions, actions are source of information for store that will be sent with dispatch, they send data from app to store
Store, will take in a single reducer that will then represent the state of our application globally

- [X] What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state is state that multiple components need access to, component state is state only 1 or very few components need access to.

- [X] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

It allows us to write action creators that return functions instead of an action.

- [X] What is your favorite state management system you've learned and this sprint? Please explain why!

Context API, its lightweight, its not hard to implement and it does the job.
